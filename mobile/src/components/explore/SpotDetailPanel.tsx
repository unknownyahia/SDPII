...
import type { DiscoveryAction, DiscoverySpot } from '../../types/discovery';
...
export function SpotDetailPanel({
  ...
  const primaryFact = spot.facts[0] ?? null;

  return (
    <View style={styles.stack}>
      {banner}
      <DiscoveryHeroImage hero={spot.hero} height={compact ? 132 : 172} />
      <View style={styles.copy}>
        ...
        >
          {spot.categoryLabel} · {spot.areaLabel} · {spot.distanceLabel}
        </Text>
      </View>
      <TrustSignalRow signals={spot.trustSignals} limit={compact ? 1 : 3} />
      {spot.socialSignal ? (
        <Text
          style={[
            styles.social,
      ...
      {compact ? (
        ...
        <View style={styles.factsBlock}>
          ...
          >
            {t('explore.spotFactsTitle')}
          </Text>
          <View style={styles.facts}>
            {spot.facts.map(fact => (
              <InfoRow
                key={`${spot.postId}-${fact.label}`}
                label={fact.label}
                value={fact.value}
                subtle={fact.subtle}
              />
